...
def category_exists(category_id: int): new*
...
router.get(path = '/recipes/', response_model=List[Recipe]) new .
...
return Recipe(id = recipe_id, name= recipe.name, description = recipe.description, ingredients = recipe.ingredients, instructions = recipe.instructions,
             difficulty = cuisine-recipe.cuisine, difficulty recipe.difficulty, category_id = recipe.category_id)
