...
@app.get(path="/movies/", response_model=List[Movie])  # new
def read_movies():
    return database.read_movies()

@app.get(path="/movies/{movie_id}", response_model=Movie)  # new
...
@app.put(path="/movies/{movie_id}", response_model=Movie)  # new
